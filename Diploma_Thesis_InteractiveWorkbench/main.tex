...
\section{Abstract}
\subsection{English}
The diploma thesis "Interactive Workbench" documents the design, development, and evaluation of a touch-optimized, unified digital environment intended to simplify and consolidate practical workflows on touch-enabled devices. Modern graphical user interfaces were designed around mouse and keyboard interaction, a paradigm that introduces significant friction when applied to touchscreen hardware. The Interactive Workbench addresses this limitation by providing a modular platform in which a range of self-developed applications — including a 3D model viewer, AI assistant, code and text editor, Markdown editor, paint application, music library, image editor, and game collection — are accessible through a single coherent interface centered on gesture-based input.
The system is implemented as a cross-platform web application using React and JavaScript for the frontend, with a Python-based backend handling AI inference and voice recognition through a locally executed large language model. Communication between the frontend and backend is managed through a FastAPI interface and WebSocket connection, while Electron provides the desktop application layer enabling access to native operating system functionality. The workbench runs on Windows, macOS, and Linux without platform-specific adaptations, and was developed and tested using the WebStorm IDE across multiple hardware configurations.
Development followed an agile methodology based on Scrum principles, structured into weekly iterations with continuous evaluation and refinement. Task distribution was managed through GitHub, which served as the central platform for version control, project planning, and progress tracking. The two-person development team divided responsibilities according to individual strengths while collaborating on architecturally complex components.
Key technical challenges encountered during development included establishing stable cross-environment communication between the JavaScript frontend and Python backend, implementing consistent touch input support across applications with fundamentally different interaction models, and ensuring reliable cross-platform behavior across all supported operating systems. External open-source applications — OrcaSlicer, FreeCAD, and Bambu Studio — were integrated as native process launches to provide professional-grade 3D slicing and CAD functionality without replicating their complexity within the workbench itself.
All data processing, including AI inference and voice input, is executed locally on the user's device. No personal data is collected or transmitted to external servers, ensuring a sufficient standard of privacy and security for the intended use cases of private, educational, and maker-space environments. Iterative user testing validated the system's usability, yielding task completion rates above 95 percent and response times under ten seconds in final benchmarks.
The thesis concludes with a reflective discussion of the development process, an honest assessment of the current implementation's limitations, and a set of concrete recommendations for future improvements, including extended theme support, improved AI performance, bidirectional external application integration, persistent game score storage, and expanded file format support in the 3D Viewer.

\subsection{German}
Die Diplomarbeit „Interactive Workbench" dokumentiert den Entwurf, die Entwicklung und die Evaluierung einer touchoptimierten, einheitlichen digitalen Arbeitsumgebung, die darauf abzielt, praktische Arbeitsabläufe auf berührungsempfindlichen Geräten zu vereinfachen und zu konsolidieren. Moderne grafische Benutzeroberflächen wurden primär für die Interaktion mit Maus und Tastatur konzipiert – ein Paradigma, das bei der Verwendung auf Touchscreen-Hardware zu erheblichen Nutzungseinschränkungen führt. Die Interactive Workbench begegnet dieser Problematik durch eine modulare Plattform, auf der eine Reihe selbst entwickelter Anwendungen – darunter ein 3D-Modell-Viewer, ein KI-Assistent, ein Code- und Texteditor, ein Markdown-Editor, eine Zeichenanwendung, eine Musikbibliothek, ein Bildeditor sowie eine Spielesammlung – über eine einheitliche, gestenbasierte Benutzeroberfläche zugänglich sind.
Das System ist als plattformübergreifende Webanwendung implementiert, wobei React und JavaScript für das Frontend eingesetzt werden und ein Python-basiertes Backend die KI-Inferenz sowie die Spracherkennung über ein lokal ausgeführtes Large Language Model übernimmt. Die Kommunikation zwischen Frontend und Backend erfolgt über eine FastAPI-Schnittstelle und eine WebSocket-Verbindung, während Electron als Desktop-Anwendungsschicht den Zugriff auf native Betriebssystemfunktionen ermöglicht. Die Workbench läuft ohne plattformspezifische Anpassungen unter Windows, macOS und Linux und wurde mithilfe der WebStorm IDE auf mehreren Hardwarekonfigurationen entwickelt und getestet.
Die Entwicklung folgte einer agilen Methodik auf Basis von Scrum-Prinzipien, strukturiert in wöchentliche Iterationen mit kontinuierlicher Evaluierung und Verfeinerung. Die Aufgabenverteilung wurde über GitHub verwaltet, das als zentrale Plattform für Versionskontrolle, Projektplanung und Fortschrittsverfolgung diente. Das zweiköpfige Entwicklungsteam teilte die Verantwortlichkeiten entsprechend der individuellen Stärken auf und arbeitete bei architektonisch komplexen Komponenten gemeinsam.
Zu den wesentlichen technischen Herausforderungen zählten die Herstellung einer stabilen Kommunikation zwischen dem JavaScript-Frontend und dem Python-Backend, die einheitliche Implementierung von Touch-Eingabeunterstützung über Anwendungen mit grundlegend unterschiedlichen Interaktionsmodellen hinweg sowie die Sicherstellung eines zuverlässigen plattformübergreifenden Verhaltens auf allen unterstützten Betriebssystemen. Externe Open-Source-Anwendungen – OrcaSlicer, FreeCAD und Bambu Studio – wurden als native Prozessstarts integriert, um professionelle 3D-Slicing- und CAD-Funktionalität bereitzustellen, ohne deren Komplexität innerhalb der Workbench selbst abbilden zu müssen.
Die gesamte Datenverarbeitung, einschließlich KI-Inferenz und Spracheingabe, erfolgt lokal auf dem Gerät des Benutzers. Es werden keine personenbezogenen Daten erfasst oder an externe Server übertragen, wodurch ein ausreichendes Maß an Datenschutz und Datensicherheit für die vorgesehenen Einsatzbereiche in privaten, schulischen und Maker-Space-Umgebungen gewährleistet wird. Iterative Benutzertests bestätigten die Gebrauchstauglichkeit des Systems mit Aufgabenabschlussraten von über 95 Prozent und Antwortzeiten von unter zehn Sekunden in abschließenden Messungen.
Die Arbeit schließt mit einer reflektierenden Diskussion des Entwicklungsprozesses, einer ehrlichen Bewertung der Einschränkungen der aktuellen Implementierung sowie einer Reihe konkreter Empfehlungen für zukünftige Verbesserungen, darunter erweiterter Designunterstützung, verbesserter KI-Leistung, bidirektionaler Integration externer Anwendungen, persistenter Spielstandspeicherung und erweiterter Dateiformatunterstützung im 3D-Viewer.

% Hauptteil basierend auf Gliederung
\section{General Introduction}
\subsection{Introduction}
Modern graphical user interfaces were designed primarily for mouse and keyboard interaction, a paradigm that has remained largely unchanged for decades. While this model is well-suited to traditional desktop environments, it introduces significant friction when applied to touchscreen devices, where layered menus, small click targets, and complex navigation hierarchies translate poorly to finger-based input. As touch-enabled laptops and interactive displays become increasingly common in both private and professional settings, the demand for interfaces that treat touch as a first-class input method — rather than an afterthought — continues to grow.
...
The "Interactive Workbench" was developed to address both of these problems. The goal was to create a touch-optimized, unified digital environment that consolidates practical tools into a single coherent interface, reducing both the friction of touch interaction and the overhead of switching between applications. The system was designed from the ground up with gesture-based input as the primary interaction model, while maintaining full compatibility with conventional mouse and keyboard input for devices without touchscreen support.
...
User interaction is centered on gesture-based input — including taps, swipes, pinches, and drag operations — supported consistently across all integrated applications. For devices without touchscreen capability, full mouse and keyboard support ensures accessibility across different hardware configurations.

Cross-platform stability and usability were validated through iterative testing with independent users across a range of scenarios, including multi-gesture sequences, AI-assisted feature calls, and concurrent application loads. Issues identified during testing — including gesture recognition inconsistencies, cross-platform rendering differences, and inter-process communication errors — were systematically addressed throughout development. Final benchmarks recorded task completion rates above 95 percent and response times under ten seconds across tested workflows.
