...
% Titelblatt-Setup
\title{DIPLOMARBEIT \\ Interactive Workbench}
\author{
  Ausgeführt im Schuljahr 2025/26 \\
  von \\
  Noah Maringer 5BHEL \\
...
\date{Vorgelegt am 07. April 2026}
...
The diploma thesis “Interactive Workbench” is a new and improved way to interact with technology at the touch of your fingers. 
Current graphical user interfaces(GUI) are clumsy and not user friendly with way too many layers of inputs which also makes them hard to use with touchscreen devices. This diploma thesis is meant to improve that by bringing to live a new touch-optimized GUI that simplifies workflows by minimizing layers and enabling intuitive interactions while maintaining functionality and portability. 

Built with React and JavaScript for the frontend, Ollama AI for smart features, and a backend for fast and seamless operations, it runs on touch- enabled laptops across Windows, macOS and Linux.

\subsection{Achieved Specifications}
These apps cover a broad spectrum of practical domains, from production planning and 3D design tools for rapid prototyping, to software development environments and even casual games for downtime entertainment.
Users engage through intuitive, simplified gestures—like swipes, pinches, and taps—across minimal interface layers, creating seamless end-to-end workflows from initial concept sketching to full execution.
The workbench supports cross-platform compatibility on Windows, macOS, and Linux, developed and debugged using WebStorm IDE, ensuring consistent performance on various touch-sensitive laptops without platform-specific glitches. Reliable usability was validated through rigorous, iterative testing with independent users in high-risk failure scenarios, such as multi-gesture sequences, AI-assisted feature calls via Ollama LLM, and heavy app loads.
Early tests uncovered issues in app compatibility, feature responsiveness (e.g., drag-and-drop gestures), and cross-OS stability, which were systematically resolved—resulting in a more robust version with 95 +percent task completion rates and under 10-second response times in final benchmarks.
This process followed standard UI testing best practices, including task-oriented sessions, gesture verification, and think-aloud protocols, confirming the GUI's intuitive design and low error rates for both experts and novices.
...
\begin{enumerate}
    ...
    \item If it works you can clone the .bat file onto your desktop for example and change its content to fit your system. 
...
% Füge hier Inhalte ein
This header provides interesting information about different technical relevant topics and hence is that important. techniques used to fulfill our main task.
...
In terms of extensibility, the workbench provides an excellent platform. Due to its application oriented architecture, the system can easily be extended with new applications, system improvements and additional features. With this in mind, the system can be extended without requiring a complete rewrite or make major changes to the existing system, which support long term scalability. The use of modern and widely adopted technologies ensures future development for developers and students with a low entry barrier. 
...
\paragraph{Current Limitations}\mbox{} \\
The biggest limitation to the performance to the ai is something that we value just as much as performance, and that is security. As a consequent of running everything locally on the users device, we lose a lot of performance as the LLM can take up to about 5GB of RAM. This means that, on lower end machines, if the ai runs, it will run relatively slow. On the other hand, assured this approach a higher level of security and safety. Another limiting factor is cost. Using well performing models with a high count of parameters, not only consumes more hardware, but is also expensive because of licensing costs. 
...
These refinements contribute to a stable and responsive editing experience while maintaining compatibility with the broader architecture of the Electron-based workbench.
...
\newpage
\section{Discussion}
The final installment of our Workbench offers a wide variety of applications and use cases, ranging from 3D manufacturing and software development to a number of other practical functionalities, making it a versatile and capable platform for its intended environment.

Looking back along the path of development however, there were several events and situations that had us reconsider our choices and decisions, requiring the team to adapt and adjust the direction of the project at various points throughout the process. While the development journey was not without its difficulties, it is worth noting that there were many problems we encountered, but none that we were ultimately not able to work through. Every challenge that arose was met and resolved in one way or another, which speaks to the dedication and persistence of the team involved.

Time proved to be one of the most valuable and limited resources throughout the entire development process. Every decision that led us to an unexpected problem forced us to spend additional time on bug fixing and troubleshooting, which in turn stole time that could have been used more productively elsewhere in the development. This was particularly felt during the development of certain applications, where the addition of a single new feature would often cause two or more new bugs to appear in its place. This pattern made the development noticeably slower and considerably more exhausting at times, as progress felt difficult to sustain when every step forward seemed to bring new obstacles along with it.

As expected with such a big and complex project, motivation and teamwork play a major part in the successfull development, even when it comes to the smallest projects.
Nevertheless, a project of such complexity and extent needs to be treated as such. Thankfully, we prepared for such events as the previously mentioned "Division of tasks" helped us not lose intereset as there was always something to improve, bug- fix or something completely new to develop. 
On the other hand, no project nor teamwork is a smooth sailing experience, which we also noticed in ways like a lack of variety in work, which came progressivly more tiring and boring. 

Attendance and health also took noticeable amount of time from us, although we managed to recover some of that time by working overtime in private. 
...
During the development process, several technical challenges emerged that significantly influenced both workflow and time management.

One of the primary difficulties concerned frontend styling. As outlined in Section 5.1.2 “Frontend Software Development”, the initial implementation did not consistently rely on structured CSS from the beginning. Instead, styling decisions evolved incrementally during development. This approach led to inconsistencies and additional refactoring effort later in the project. A more systematic styling strategy from the outset would have reduced redundancy and improved development efficiency.

Another major challenge involved the integration of the AI assistant. The implementation required interaction between JavaScript-based frontend components and Python-based backend processes. This cross-environment communication resulted in recurring issues, including connection errors and execution inconsistencies when running Python scripts. Resolving these problems required substantial debugging effort and resource allocation, particularly in ensuring stable inter-process communication.

The biggest challenge faced in the development of our workbench was the idea to make everything touch input capable, which led to numerous problems.
This idea challenged us to find ways to interact with the workbench in ways that some would say are unnatural such as buttons where most would expect a slider or something similar. 
This can be seen in the byte invaders game where this idea forced us to develop a new movement methodology, moving away from event triggered movements through swipes and simple presses to buttons whom can be activated via touch inputs.

Portability and compatibility also slowed down the development as they requiered intensiv care and caution to guarantee a stable project on both windows and other platforms. More on this can be found in the section "Technical Specifications". 

Furthermore and definitely worth mentioning, were the problems we encountered with GitHub. GitHub, while being a major part of planning, managing and hosting our project, did not come without it problems. As our project grew, we resorted to hosting both a GitHub repository as well as a project. At first, we did not give them enough of credit and care which later let to issues when merging our rather long-lived development branches, which at that point were to different and required manual merging which led to problems more than once. 

\newpage
\subsection{Review of the Project Realization}
Looking back at the development process, several of the challenges encountered were attributable to limited prior experience with the chosen technology stack. The learning curve associated with JavaScript, React, CSS, and cross-process communication in Electron contributed to inefficiencies during the earlier phases of the project. In hindsight, a more thorough evaluation of architectural decisions — particularly around frontend structuring and AI integration — at the planning stage would have reduced the amount of later refactoring required. The GitHub workflow issues similarly reflect a maturation process: as the project grew in complexity, the team's approach to branch management and merging strategy evolved, though not always without friction. Despite these setbacks, the iterative methodology allowed the project to recover from most obstacles without compromising the final result. The workbench as delivered fulfills the core functional and architectural requirements defined at the outset, and the development process represents a substantial gain in practical engineering experience for both contributors.

One of the primary difficulties concerned frontend styling. As outlined in Section 5.1.2 “Frontend Software Development”, the initial implementation did not consistently rely on structured CSS from the beginning. Instead, styling decisions evolved incrementally during development. This approach led to inconsistencies and additional refactoring effort later in the project. A more systematic styling strategy from the outset would have reduced redundancy and improved development efficiency.

Another major challenge involved the integration of the AI assistant. The implementation required interaction between JavaScript-based frontend components and Python-based backend processes. This cross-environment communication resulted in recurring issues, including connection errors and execution inconsistencies when running Python scripts. Resolving these problems required substantial debugging effort and resource allocation, particularly in ensuring stable inter-process communication.

These technical issues extended development time but also provided valuable insights into system architecture and dependency management.

From a reflective perspective, several of the encountered issues can be attributed to limited prior experience with JavaScript, React, and CSS. While the project significantly improved our technical competence in these areas, the learning curve contributed to inefficiencies during early development phases.

Additionally, some challenges indicate that the initial verification and evaluation of development strategies could have been more rigorous. A more detailed assessment of architectural decisions — particularly regarding frontend structuring and AI integration — might have reduced later restructuring efforts.

As previously mentioned, we also had to put up with GitHub. From a reflective perspective, we agreed that these issues came both from a lack of experience working with GitHub as well as our ambition to host a project and repository as good as can be. 
...
Recommendations and improvements are always welcome and necessary. 

One of these improvements for further development could be that the design which you chose in the settings, applies not only to the app icons but also to the applications themselves.

Furthermore, at its current state, the AI is slow and sluggish. An overhaul of the AI could make the whole workbench more futuristic and future proof while also making it more usable and providing the user with better information and help. However, with future improvements the developer should not forget certain safety goals defined in earlier stages, ensuring that data security will always be guaranteed.
We want to mention here that at the current point of time, there is a new and improved version of the ai assistant in private development. 

Another improvement comes with the performance of the entire workbench. 
When run on mid- tier performance machines, the workbench performs satisfactory, but noticeable slower than on a higher-tier performance machine. We also noticed a difference in performance between different operating systems, in particular with MacOS and Windows. 
This could be improved with further versions of npm and React. 
Supplementary to that, the applications could be rewrote in a more performance oriented way. 

One of the biggest technical challenges the workbench has to deal with is the problem with hard coded paths used for the start up of extern applications like the slicers implemented in the workbench. At the current state, the paths used are the standard installation paths, which means that the workbench will fail to launch these apps on any device using different paths. This could be fixed by adding some sort of automatic path finder or a user configurable path entry setting.

Another improvement that could be made to ease out the work process of adding games to the workbench would be to change the current architecture behind how the game collection links up to the games and the app. Currently, adding a new game involves manual changes to both the app and the game collection. As the collection grows, this pattern becomes increasingly fragile and error-prone. A more maintainable approach would be a self-registering plugin system where each game declares its own metadata — name, ID, cover image, description — and the collection assembles itself dynamically at runtime.
Also concerning the games and the game collection, things like score persistence and player progression are entirely absent from the games. There are no high scores, no personal bests, and no sense of progression between sessions. Given that the workbench already runs in Electron, which has access to the local filesystem, implementing persistent score storage via a simple JSON file or localStorage would be straightforward and would significantly improve the replay value of the Game Collection.

Furthermore, file format export flexibility in the 3D Viewer is currently limited to GLTF regardless of the input format. A user who imports an STL file — the dominant format in the 3D printing workflow the workbench is built around — will receive a GLTF file on export, which may not be directly usable in their slicer without conversion. Preserving or offering the original format on export, particularly STL, would make the viewer a much more practical part of the 3D printing workflow.

Also worth mentioning is the fact that the external application integrations could be made bidirectional. Currently the workbench can launch OrcaSlicer, FreeCAD, and Bambu Studio, but has no awareness of what the user does inside those applications, whether a file was saved, a print was started, or an error occurred. Deeper integration — for example, watching a known output directory for new files and maybe offering them as a shortcut when the user enters the 3D- Viewer, or reading a slicer's log output — would make the workbench feel even more like a hub to center all your productivity rather than a launcher.

\newpage
\section{Directories}
\subsection{List of illustrations}    

Illustration 1 : Home Screen presented to the use after start-up \dotfill 6\newline
Illustration 2 : Block diagram showing the systems architecture and input handling \dotfill9\newline
Illustration 3 : Block diagram showing the systems capabilities and workflow         \dotfill13\newline
Illustration 4 : Team capacity and division of tasks                                 \dotfill15\newline
Illustration 5 : GitHub Development Roadmap \dotfill17
Illustration 6 : Paint applications \dotfill21
Illustration 7: AI- Assistant: byte assistant \dotfill24
Illustration 8: Text- and Code- Editor\dotfill26
Illustration 9: Markdown Editor\dotfill30
Illustration 10: 3D- Viewer displaying an object\dotfill33
Illustration 11: The music library playing a song by a certain swedish band\dotfill35
Illustration 12: The GameCollection\dotfill13
Illustration 13: Overview of all applications\dotfill45

        
\subsection{List of tables}

\section{Attachment}
\subsection{Used documentations}
\begin{itemize}
    \item Node.js: https://nodejs.org/docs/latest/api/
    \item Python: https://www.python.org/doc/
\end{itemize}
...
I hereby declare,
that I have written this diploma thesis independently and without external assistance. This diploma thesis was completed without the use of any sources or aids other than those cited. Any content taken verbatim or in paraphrased form from publications or other sources has been identified as such.
The work has not been published in the same or similar form and has not been submitted to any examining authority.
